...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    l2 = np.maximum(0,l1)
    l3 = l2.dot(W2) + b2
    l4 = np.exp(l3 - np.max(l3, axis = 1)[:,np.newaxis])
    rows = np.sum(l4, axis = 1)
    l4 = l4 / rows[:, np.newaxis]
    ...
    loss = np.sum(-np.log(l4[range(N), y]))
    loss /= N
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in xrange(num_iters):
      ...
      if verbose and it % 1000 == 0:
        print 'iteration %5d / %5d: loss %8f' % (it, num_iters, loss)
